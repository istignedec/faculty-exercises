...
int main() {
	...
	// ZLATNI REZ
	double a, b, h = 0.1, tocka = 10, eps = 0.000001;
	funkcijaZad1 fja1;

	double x = zlatni_rez_tocka(h, tocka, &a, &b, eps, fja1);
	printf("\nInterval = [%f, %f], x = %f\n", a, b, x);
	...
	h = 0.1;
	int n = 1;
	x0.push_back(10); // pocetna tocka
	e.push_back(0.000001);

	funkcijaZad1 fja2;
	vector<double> xmin;

	xmin = koordinatno_trazenje(x0, e, h, fja2);
	...
	n = 1;
	vector<double> x0_3;
	x0_3.push_back(10); // pocetna tocka
	vector<double> Dx;
	Dx.push_back(0.5);
	vector<double> e_3;
	...
	min = hooke_jeeves(x0_3, Dx, e_3, fja3);
	...
	n = 1;
	vector<double> x0_4;
	x0_4.push_back(10); // pocetna tocka
	double e4 = 0.000001, pomak = 0.01, alfa = 1, beta = 0.5, gamma = 2, sigma = 0.5;
	vector<double> min_4;

	min_4 = simplex(x0_4, e4, pomak, alfa, beta, gamma, sigma, fja4);
	printf("\n\nPronadjena tocka minimuma :\n");
	for (int i = 0; i < n; i++) {
		printf(" %f", min_4[i]);
	}
	...
	int poziv1 = fja1.getBr();
	int poziv2 = fja2.getBr();
	int poziv3 = fja3.getBr();
	int poziv4 = fja4.getBr();
	printf("\nBroj poziva - ZLATNI REZ : %d", poziv1);
	printf("\nBroj poziva - PRETRAGA PO KOORD. OSIMA : %d", poziv2);
	printf("\nBroj poziva - HJ : %d", poziv3);
	printf("\nBroj poziva - SIMPLEX : %d", poziv4);
	printf("\n");
	return 0;
}