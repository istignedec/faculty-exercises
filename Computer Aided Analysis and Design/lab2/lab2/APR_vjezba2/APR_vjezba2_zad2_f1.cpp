...
int main() {
	...
	// KOORDINATNO TRAŽENJE FUNKCIJA 1
	vector<double> x0;
	vector<double> e;
	
	double h = 0.1;
	int n = 2;
	x0.push_back(-1.9); // pocetna tocka
	x0.push_back(2);
	e.push_back(0.0001);
	e.push_back(0.0001);
	...
	xmin = koordinatno_trazenje(x0, e, h, fja11);
	printf("\nKoordinate tocke minimuma FUNKCIJA 1, PO KOORDINATNIM OSIMA:\n");
	for (int i = 0; i < n; i++) {
		printf("%f ", xmin[i]);
	}
	
	// HOOKE-JEEVES POSTUPAK FUNKCIJA 1

	funkcija1 fja12;
	n = 2;
	vector<double> x0_3;
	x0_3.push_back(-1.9); // pocetna tocka
	x0_3.push_back(2);
	vector<double> Dx;
	Dx.push_back(0.5);
	Dx.push_back(0.5);
	...
	min = hooke_jeeves(x0_3, Dx, e_3, fja12);
	printf("\n\nPronadjena tocka minimuma FUNKCIJA 1, POSTUPAK HJ:\n");
	for (int i = 0; i < n; i++) {
		printf(" %f", min[i]);
	}
	printf("\n");

	// SIMPLEX POSTUPAK FUNKCIJA 1

	funkcija1 fja13;
	n = 2;
	vector<double> x0_4;
	x0_4.push_back(-1.9); // pocetna tocka
	x0_4.push_back(2);
	double e4 = 0.000001, pomak = 0.01, alfa = 1, beta = 0.5, gamma = 2, sigma = 0.5;
	vector<double> min_4;

	min_4 = simplex(x0_4, e4, pomak, alfa, beta, gamma, sigma, fja13);
	printf("\n\nPronadjena tocka minimuma, FUNKCIJA 1 SIMPLEX :\n");
	for (int i = 0; i < n; i++) {
		printf(" %f", min_4[i]);
	}
	printf("\n");
	...
	int poziv1f1 = fja11.getBr();
	int poziv2f1 = fja12.getBr();
	int poziv3f1 = fja13.getBr();

	printf("\nBroj poziva funkcije 1- PRETRAGA PO KOORD. OSIMA : %d", poziv1f1);
	printf("\nBroj poziva funkcije 1- HJ : %d", poziv2f1);
	printf("\nBroj poziva funkcije 1- SIMPLEX : %d", poziv3f1);
	printf("\n");
