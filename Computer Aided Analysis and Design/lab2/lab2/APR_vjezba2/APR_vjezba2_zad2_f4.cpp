...
int main() {
	...
	// KOORDINATNO TRAŽENJE FUNKCIJA 4
	vector<double> x0;
	vector<double> e;
	
	double h = 0.1;
	int n = 2;
	x0.push_back(5.1); // pocetna tocka
	x0.push_back(1.1);
	...
	xmin = koordinatno_trazenje(x0, e, h, fja41);
	printf("\nKoordinate tocke minimuma FUNKCIJA 4, PO KOORDINATNIM OSIMA:\n");
	for (int i = 0; i < n; i++) {
		printf("%f ", xmin[i]);
	}
	
	// HOOKE-JEEVES POSTUPAK FUNKCIJA 4

	funkcija4 fja42;
	n = 2;
	vector<double> x0_3;
	x0_3.push_back(5.1); // pocetna tocka
	x0_3.push_back(1.1);
	vector<double> Dx;
	Dx.push_back(0.5);
	Dx.push_back(0.5);
	...
	min = hooke_jeeves(x0_3, Dx, e_3, fja42);
	printf("\n\nPronadjena tocka minimuma FUNKCIJA 4, POSTUPAK HJ:\n");
	for (int i = 0; i < n; i++) {
		printf(" %f", min[i]);
	}
	printf("\n");

	// SIMPLEX POSTUPAK FUNKCIJA 4

	funkcija4 fja43;
	n = 2;
	vector<double> x0_4;
	x0_4.push_back(0); // pocetna tocka
	x0_4.push_back(0);
	double e4 = 0.000001, pomak = 0.01, alfa = 1, beta = 0.5, gamma = 2, sigma = 0.5;
	vector<double> min_4;

	min_4 = simplex(x0_4, e4, pomak, alfa, beta, gamma, sigma, fja43);
	printf("\n\nPronadjena tocka minimuma, FUNKCIJA 4 SIMPLEX :\n");
	for (int i = 0; i < n; i++) {
		printf(" %f", min_4[i]);
	}
	printf("\n");
	...
	int poziv1f4 = fja41.getBr();
	int poziv2f4 = fja42.getBr();
	int poziv3f4 = fja43.getBr();

	printf("\nBroj poziva funkcije 4- PRETRAGA PO KOORD. OSIMA : %d", poziv1f4);
	printf("\nBroj poziva funkcije 4- HJ : %d", poziv2f4);
	printf("\nBroj poziva funkcije 4- SIMPLEX : %d", poziv3f4);
	printf("\n");
