...
int main() {
	...
	double n = 2;
	vector<double> x0;
	x0.push_back(0.5); // pocetna tocka
	x0.push_back(0.5);
	double e = 0.000001, alfa = 1, beta = 0.5, gamma = 2, sigma = 0.5;
	vector<double> min;

	for (int pomak = 1; pomak <= 20; pomak++) {
		min = simplex(x0, e, pomak, alfa, beta, gamma, sigma, fja);
		printf("\n\nPronadjena tocka minimuma za pomak %d, FUNKCIJA 4 SIMPLEX :\n", pomak);
		for (int i = 0; i < n; i++) {
			printf(" %f", min[i]);
		}
		printf("\n");
		...
		int poziv = fja.getBr();
		printf("\nBroj poziva funkcije 1 za pomak %d - SIMPLEX : %d", pomak, poziv);
		printf("\n");
	}
