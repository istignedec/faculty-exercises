...
int main() {
	...
	// SIMPLEX POSTUPAK FUNKCIJA 6

	funkcija6 fja;
	double n = 2;
	double e = 0.000001, alfa = 1, pomak = 1, beta = 0.5, gamma = 2, sigma = 0.5;
	vector<double> min;
	srand((unsigned)time(NULL));

	for (int i = 0; i < 10; i++) {
		int randX = rand() % (50 + 50 + 1) - 50;
		int randY = rand() % (50 + 50 + 1) - 50;
		vector<double> x0;
		x0.push_back((double)randX);
		x0.push_back((double)randY);

		min = simplex(x0, e, pomak, alfa, beta, gamma, sigma, fja);
		printf("\n\nPronadjena tocka minimuma za pomak %f, FUNKCIJA 6 SIMPLEX, pocetna tocka (%d,%d) :\n", pomak, randX, randY);
		for (int i = 0; i < n; i++)
			printf(" %f", min[i]);

		int poziv = fja.getBr();
		printf("\nBroj poziva funkcije 6 za pomak %f - SIMPLEX : %d", pomak, poziv);
		printf("\n");
	}
	return 0;
}