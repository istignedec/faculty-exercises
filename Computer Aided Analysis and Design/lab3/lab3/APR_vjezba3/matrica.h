...
public:
	...
	Matrica(unsigned brRedaka, unsigned brStupaca, vector<double> vrijednost) {
		m_brRedaka = brRedaka;
		m_brStupaca = brStupaca;
	...
	Matrica operator+(Matrica &B) {
		Matrica sum(m_brStupaca, m_brRedaka, 0.0);
	...
	Matrica operator-(Matrica & B) {
		Matrica diff(m_brStupaca, m_brRedaka, 0.0);
		for (unsigned i = 0; i < m_brRedaka; i++) {
			for (unsigned j = 0; j < m_brStupaca; j++) {
	...
	Matrica operator~() {
		Matrica Transpose(m_brStupaca, m_brRedaka, 0.0);
...
Matrica zamjenaRedaka(Matrica A, Matrica P) {
	Matrica pom(A.getBrRedaka(), A.getBrStupaca(), 0.0);
	for (unsigned i = 0; i < A.getBrRedaka(); i++) {
		unsigned indexRetka = P(i, 0);
		for (unsigned j = 0; j < A.getBrStupaca(); j++) {
			pom(i, j) = A(indexRetka, j);
		}
	}
	return pom;
}