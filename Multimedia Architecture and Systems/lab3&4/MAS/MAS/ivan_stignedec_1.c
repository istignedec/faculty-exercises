/* KODIRANJE */
/* Program napisan i kompajliran unutar MS Visual Studio 2017 */
	...
	for (i = 0; i < N; i += 8) {
		for (j = 0; j < M; j += 8) {
			for (k = 0; k < 3; k++) {
				...
				/* pretvori f i F u flat matrice tipa Ipp32f* */

				int pStepBytes = 0;
				Ipp32f * flatf = ippiMalloc_32f_C1(8, 8, &pStepBytes);
				for (int i = 0; i < 8; i++) {
					for (int j = 0; j < 8; j++) {
						flatf[i * 8 + j] = f[i][j];
					}
				...
				Ipp32f * flatF = ippiMalloc_32f_C1(8, 8, &pStepBytes);
				ippiDCT8x8Fwd_32f_C1(flatf, flatF); /* DCT(f, F); */
