...
/**
   ...
   */
  public Vector getReflectedVector(Vector N) {
	  // odbijeni vektor R=(2U*N)*N-U
  ...
  public Vector getRefractedVector(Vector N, double nI) {
	  ...
	  alfa = this.getAngle(N);
	  cosA = Math.cos(alfa);

	  // a i b se racunaju preko snellovog zakona, a=a=(-2*b*cosA+sqrt(D))/2, b=n1/n2, n2=1
	  b = nI;
	  D = 4*(b*b*cosA*cosA-b*b+1);
	  a = (-2*b*cosA+Math.sqrt(D))/2;

	  // refraktirana zraka R=-a*N-b*U
	  R = N.multiple(-a).sub(U.multiple(b));
	  return R;
  }
}