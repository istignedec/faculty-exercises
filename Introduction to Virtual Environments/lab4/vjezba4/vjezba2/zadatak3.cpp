...
void display(void)
{
	...
	*/
	...
	glPushMatrix();
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glCallList(YELLOWMAT); // boja
	drawSphere(6.96, 0.01);
	glPopMatrix();

	// Zemlja
	glPushMatrix();
	glCallList(BLUEMAT); // boja
	glRotated(earthYear, 0.0, 1.0, 0.0); //parametri iz spinDisplay funkcije

	glTranslated(0.0, 0.0, 14.96); // udaljenost Zemlje i Sunca
	glPushMatrix();
	glRotated(-earthDay, 0.0, 1.0, 0.0); // rotacija u suprotnom smjeru
	drawSphere(0.63, 0.1); // promjer Zemlje, step
	glPopMatrix();

	// Mjesec
	glPushMatrix();
	glCallList(WHITEMAT); // boja
	glRotated(moonYear, 0.0, 1.0, 0.0);
	glTranslated(0.0, 0.0, 1.0); // udaljenost Zemlje i Mjeseca
	glPushMatrix();
	glRotated(-moonDay, 0.0, 1.0, 0.0); // rotacija u suprotnom smjeru
	drawSphere(0.17, 0.1); // promjer Mjeseca, step
	glPopMatrix();
	glPopMatrix();
	glPopMatrix();
	glPopMatrix();

	glutSwapBuffers(); // crtaj iz drugog spremnika slike
}
