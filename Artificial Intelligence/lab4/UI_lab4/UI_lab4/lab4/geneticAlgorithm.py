...
class GeneticAlgorithm(object): 
	...
	def step(self):	
		...
		while len(newPopulation) < self.populationSize:
			parentOne, parentTwo = self.selectParents()
			baby = self.crossover(parentOne, parentTwo)
			chromosome = np.array(self.mutate(baby))
			fitness = self.calculateFitness(chromosome)
			newPopulation.append((chromosome, fitness))

		stop = self.best() < self.e or self.i > self.numIter
		self.population = sorted(newPopulation, key=lambda t: -t[1])

		return stop, self.i, self.best()[0]
	...
	def selectParents(self):
		...
		fitnessSum = 0
		for fit in self.population:
			fitnessSum += fit[1]

		parentOne = self.select(fitnessSum)
		parentTwo = self.select(fitnessSum)
		return parentOne, parentTwo

	def select(self, fitnessSum):
		currFit = 0
